...
export class RacingLine {
   ...
   */
  static fromInput(raw: RawSample[], maxSpeed: number, closed = true): RacingLine {
    ...
    const pts = smoothPositions(raw.map((r) => r.p), 2);
  ...
  at(s: number): { pos: Vec2; tan: Vec2; speed: number } {
    const n = this.nodes;
    if (n.length === 0) {
      return { pos: { x: 0, y: 0 }, tan: { x: 1, y: 0 }, speed: 0 };
    }
